...
exports.getConfigData = function(req, res) {
    ...
    fs.readFile('./config.json', {
        ...
        if (err) {
            closeLog('\n\tError retrieving the server configuration data: ' + err.message, 1);
            res.sendStatus(500);
        } else {
            closeLog('\n\tConfiguration data successfully retrieved', 3);
...
exports.updateConfigData = function(req, res) {
    ...
    fs.writeFile('./config.json', JSON.stringify(req.body), {
        ...
        if (err) {
            closeLog('\n\tError updating the server configuration file: ' + err.message, 1);
            res.sendStatus(500);
        } else {
            closeLog('\n\tServer configuration file successfully updated', 3);
            res.sendStatus(200);
        }
    });
